...
//			}
			...
			control.click(function(){
				if (switcher.css('left') == '0' || switcher.css('left') == '0px'){
					switcher.animate({ left: '-225px'}, 250);
...
//			var cookieColor = $.cookie('switcherColors');
			...
			/*if (cookieColor != undefined && cookieColor != 'orange') {
				var link = $('<link>', {rel: 'stylesheet', href: window.wp_template_dir_uri+'css/colors/'+cookieColor+'.css'}).appendTo('body');

				switcher.find('.color_orange').removeClass('active');
				switcher.find('.color_'+cookieColor).addClass('active');
			} else {
				var link = $('<link>', {rel: 'stylesheet', href: window.wp_template_dir_uri+'css/colors/orange.css'}).appendTo('body');
			}*/
				...
				$(this).click(function(){
					if ( ! $(this).hasClass('active')){
						...
						var color = $(this).attr('data-color'),
							url = window.wp_template_dir_uri+'css/colors/color_'+color+'.css';

						$('.w-logo-img').attr('src', window.wp_template_dir_uri+'img/logo_'+color+'.png');
